...
static int		cred_acquire_cb(git_cred** cred, const char* url, const char* username_from_url, unsigned int allowed_types,
                                void* payload);
...
void segmentGit(int argc, const char** argv, options_t& options, segment_t& segment) {
	...
	if (error == GIT_OK) {
		...
		std::string   stats;
		if (ahead [0] != 0 || behind [0] != 0 || ahead [1] != 0 || behind [1] != 0 || staged != 0 ||  untracked != 0 || modified != 0  || conflicts != 0 || stashed != 0) {
			stats.append(" ");
...
static int cred_acquire_cb(git_cred** cred, const char* url, const char* username_from_url, unsigned int allowed_types, void* payload) {
	return 0;
}
